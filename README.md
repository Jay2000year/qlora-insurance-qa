...
| **效果评估** | 领域内问答 F1 +18.9%（0.354 → 0.421），20 条测试用例基座 vs LoRA 对比 |
...
基于 20 条测试用例（领域内 10 条 + 领域外 10 条），对比基座模型与 LoRA 微调后的效果：

| 指标 | 基座模型 | LoRA 微调 | 变化 |
|------|---------|----------|------|
| 领域内 F1 | 0.354 | 0.421 | **+18.9%** |
| 领域外 F1 | 0.448 | 0.380 | -15.2% |
| 平均延迟 | 5.40s | 5.31s | 基本持平 |

- 领域内问答效果明显提升，说明模型学到了保险理赔领域的表达与知识
- 领域外 F1 下降，是小样本微调的典型过拟合信号，可通过混合通用数据、降低 lr、早停等方式缓解

> 注：上述评估基于早期 r=16 checkpoint（`eval_report.json`），最终最优配置 r=8/ep5（loss 1.45）建议重新评估。

### 训练过程对比

![loss comparison](output/plots/loss_comparison.png)

![training summary](output/plots/training_summary.png)
